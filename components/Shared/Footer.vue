...
/* Bir kisinin dolu e-posta alanlari */
const mailsOf = (person: Record<string, any>) =>
  [person.email, person.email2].filter(Boolean) as string[];
</script>
...
<template>
  <div class="container">
    <div class="row m-auto">
      ...
      <!-- Yetkili kisiler -->
      <div class="col-sm-4 footer-col">
        <h3 class="header">{{ footer.personal.header }}</h3>

        <div
          v-for="(person, i) of footer.personal.items"
          :key="i"
          ...
          <div class="personalContainerLimited">
            <div class="personalCardGrid">
              ...
              <div class="personalCardColumnTwo">
                <div class="personalName">{{ person.name }}</div>
                <div class="personalJob">{{ person.job }}</div>

                <div v-for="mail of mailsOf(person)" :key="mail">
                  <a :href="`mailto:${mail}`" class="personalLink">
                    <svg
                      class="personalIcon"
                      viewBox="0 0 24 24"
                      fill="none"
                    ...
                    <span class="footer-span">{{ mail }}</span>
                  </a>
                </div>

                <div v-if="person.watsappLink">
                  <a
                    :href="person.watsappLink"
                    target="_blank"
                    rel="noopener noreferrer"
                    class="personalLink"
                  >
                    ...
                    <span class="footer-span">{{ person.whatsapp }}</span>
                  </a>
                </div>
              </div>
            </div>
